...
export async function POST(request: NextRequest) {
  try {
    ...
    // Return JSON even if there's an error, never HTML
    try {
      ...
      try {
        ...
        return NextResponse.json<ApiResponse<null>>({
          success: false,
          error: 'Database connection error. Please check MongoDB URI in environment variables.',
      ...
      return NextResponse.json<ApiResponse<null>>({
        success: false,
        error: 'Server error during authentication process',
        details: innerError instanceof Error ? innerError.message : 'Unknown error'
      }, { status: 500 });
    }
  } catch (outerError) {
    console.error('Unhandled login error in outer catch:', outerError);
    // Even in the worst case, still return JSON
    return NextResponse.json<ApiResponse<null>>({
      success: false,
